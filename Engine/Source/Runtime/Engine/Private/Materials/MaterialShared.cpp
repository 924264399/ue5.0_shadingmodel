...
FText FMaterialAttributeDefinitionMap::GetAttributeOverrideForMaterial(const FGuid& AttributeID, UMaterial* Material)
{
	...
	case MP_SubsurfaceColor:
		...
		{
			return LOCTEXT("Extinction", "Extinction");// 特殊逻辑：如果是“体积材质”（MD_Volume），直接显示“Extinction”（消光/衰减）
		}
		// 给不同着色模型添加自定义命名映射
		CustomPinNames.Add({MSM_Cloth, "Fuzz Color"});// 布料着色模型→显示“绒毛颜色”
		CustomPinNames.Add({ MSM_CelToon,"CelToonSubsurfaceColor"});//ctoon//6 LYY   /卡通着色模型→显示“卡通次表面颜色”
	...
	case MP_CustomData0:	
		...
		CustomPinNames.Add({MSM_SubsurfaceProfile, "Curvature" });    //曲率
		CustomPinNames.Add({MSM_CelToon, "Range" }); //ctoon//6 LYY  // 卡通着色模型→显示“范围”（自定义卡通逻辑）
