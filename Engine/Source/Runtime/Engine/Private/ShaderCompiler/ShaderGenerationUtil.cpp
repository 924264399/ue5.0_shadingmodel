...
void FShaderCompileUtilities::ApplyFetchEnvironment(FShaderMaterialPropertyDefines& SrcDefines, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	FETCH_COMPILE_BOOL(MATERIAL_SHADINGMODEL_CELTOON); //ctoon//9 LYY
	// 作用：如果 HLSL 编译环境里有 MATERIAL_SHADINGMODEL_CELTOON 宏，会被解析到 FShaderMaterialPropertyDefines 结构体中，后续所有逻辑（比如 GBuffer 写入判断）才能用到这个标志。
...
///→ 关键！根据材质的着色模型，判断该材质需要写入 GBuffer 的哪些 Slot（比如 CustomData）。
	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	if (Mat.MATERIAL_SHADINGMODEL_CELTOON)//ctoon//9 LYY
	{
		SetStandardGBufferSlots(Slots, bWriteEmissive, bHasTangent, bHasVelocity, bHasStaticLighting, bIsStrataMaterial);
		Slots[GBS_CustomData] = bUseCustomData;
	}

	/// 作用：卡通着色模型会被标记为 “需要写入 CustomData 到 GBuffer”，从而参与 GBuffer 的编码 / 解码逻辑 —— 如果没有这行，你的卡通材质不会往 GBuffer 写自定义数据，HLSL 中卡通逻辑会缺少关键数据。
	///SetStandardGBufferSlots 这是 UE 封装的工具函数，作用是批量启用 GBuffer 的 “标准槽位”（比如世界法线、基色、粗糙度、着色模型 ID 等），避免开发者手动一行行设置 Slots[GBS_WorldNormal] = true、Slots[GBS_BaseColor] = true 等重复操作
	/// bWriteEmissive自发光   bHasTangent切线   bHasVelocity速度  bHasStaticLighting预计算阴影  bIsStrataMaterial 是否是 Strata 材质（UE 的分层渲染）   Slots[GBS_CustomData] = bUseCustomData是自定义备用槽位
