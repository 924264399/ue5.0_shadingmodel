...
////此文件执行 “材质原始标志 → 着色器编译派生标志” 的核心计算逻辑—— 把 ShaderMaterial.h 中声明的 “原始开关（比如是否是卡通着色模型、是否前向渲染）”，通过 UE 渲染管线的业务规则，推导成更具体、可直接控制 HLSL 编译的 “派生开关”，是材质→着色器编译的 “逻辑计算中枢”。
////ShaderMaterial.h 是 “开关声明手册”，这个 .cpp 文件是 “开关逻辑计算引擎”—— 前者定义了哪些开关存在，后者计算这些开关该怎么组合、该取什么值。
...
///这个函数是 UE 材质 / 着色器编译的 “规则大脑”，核心逻辑是：把零散的原始标志（材质属性、光照贴图、全局渲染、编译器），通过渲染管线的业务规则，推导成能直接控制 HLSL 编译的派生开关。
	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	//// 是否启用GBuffer（延迟渲染核心，判断条件：特性级别≥SM4、不透明/蒙版材质、非前向渲染）
	...
	// 关键：哪些着色模型需要往GBuffer写自定义数据（加的卡通着色模型需要加在这里！）
