...
///////这个文件就是个开关 的文件  就是hlsl中  宏开关的  底层根源 / 核心支撑。
///////比如HLSL 代码里的 #ifdef MATERIAL_SHADINGMODEL_CELTOON 这个宏，不是手写死在 HLSL 文件里的，而是引擎在编译着色器时，根据 C++ 侧的标志动态注入的。

struct FShaderMaterialPropertyDefines   ///	核心！材质专属开关集合
{
	...
	uint8 MATERIAL_SHADINGMODEL_CELTOON : 1;//ctoon//7 LYY
